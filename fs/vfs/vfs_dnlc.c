...
dnlc_init(void)
{
	...
	if (ncsize <= 0 || (ncache =
		(struct ncache *)kmem_zalloc(ncsize * sizeof(*ncache), KM_SLEEP)) == NULL) {
		doingcache = 0;
 ...
 */
STATIC void
nc_inshash(struct ncache *ncp, struct ncache *hp)
{
 ...
 */
STATIC void
nc_inslru(struct ncache *ncp2, struct ncache *ncp1)
{
	struct ncache *ncp3;
